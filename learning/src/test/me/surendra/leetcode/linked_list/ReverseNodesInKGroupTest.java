...
public class ReverseNodesInKGroupTest {
    ...
    private ListNode callMethod(final ListNode input, final int k) {
        return reverseNodesInKGroup.reverseKGroupUsingExtraSpace(input, k);
//        return reverseNodesInKGroup.reverseKGroup(input, k);
//        return reverseNodesInKGroup.reverseKGroupUsingStack(input, k);
    ...
    @Test
    public void test6() {
        final ListNode input = LinkedList.from(new int[]{1, 2, 3, 4, 5, 6, 7});
        final ListNode output= LinkedList.from(new int[]{5, 4, 3, 2, 1, 6, 7});

        assertThat(callMethod(input, 5), equalTo(output));
    }
}
